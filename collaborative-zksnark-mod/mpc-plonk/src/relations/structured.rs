...
impl<F: Field> PlonkCircuit<F> {
    ...
    pub fn self_test(&self) {
        ...
        assert!(self.values.is_some());
        // assert power of two
        assert!(self.n_gates().is_power_of_two());
        // test sum constraints
        for (a, b, c) in &self.sums {
            assert_eq!(
                self.values.as_ref().unwrap()[*a as usize]
                    + self.values.as_ref().unwrap()[*b as usize],
                self.values.as_ref().unwrap()[*c as usize]
            );
        }
        // test product constraints
        for (a, b, c) in &self.prods {
            assert_eq!(
                self.values.as_ref().unwrap()[*a as usize]
                    * self.values.as_ref().unwrap()[*b as usize],
                self.values.as_ref().unwrap()[*c as usize]
