...
read -t 10 -p "You have 10 seconds to think twice whether these logs should be cleared. Hit Enter to confirm, Ctrl+C to terminate..."
...
for ((i = 0; i < $CONTROL_NODE_NUM; i++)); do
    (
        ...
        ssh "$ssh_user_host" -- "rm -v ~/$PROJECT_REMOTE_DIR_NAME/$MPC_NODE_NUM/bin/log.*.txt || true"
    ) &
