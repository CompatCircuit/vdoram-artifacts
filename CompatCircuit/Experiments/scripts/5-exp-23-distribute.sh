...
(
    ...
    for ((i = 0; i < $CONTROL_NODE_NUM; i++)); do
        (
            ...
            if [ "$i" -eq 0 ]; then
                scp -- exp-constants.sh *.instance.*.json *.*.bin MpcConfig.*.json "$ssh_user_host:~/$PROJECT_REMOTE_DIR_NAME/$MPC_NODE_NUM/bin/"
            else
                scp -- exp-constants.sh *.instance."$i".json *."$i".bin MpcConfig."$i".json "$ssh_user_host:~/$PROJECT_REMOTE_DIR_NAME/$MPC_NODE_NUM/bin/"
            fi
        ) &
